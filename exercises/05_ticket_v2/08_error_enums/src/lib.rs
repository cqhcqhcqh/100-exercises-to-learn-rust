...
impl Ticket {
    ...
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError {err: "Title cannot be empty".into()});
            // return Err("Title cannot be empty".to_string());
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError {err: "Title cannot be longer than 50 bytes".into()});
            // return Err("Title cannot be longer than 50 bytes".to_string());
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError("Description cannot be empty".into()));
            // return Err("Description cannot be empty".to_string());
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError("Description cannot be longer than 500 bytes".into()));
            // return Err("Description cannot be longer than 500 bytes".to_string());
        }
